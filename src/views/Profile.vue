    ...
    <div class="orderHistory" v-if= "orders != undefined">
      ...
      class="order">
        <div class="orderFormat">
          ...
          <div v-for= "orderItem in computedOrders[index]"
          v-bind:key= "orderItem._id">
            <div class="orderFormat">
              <p>{{orderItem.title}}</p>
        ...
        for(let i = 0; i < this.orders.length; i++){
          this.computedOrders.push([]);
            for(let j = 0; j < this.orders[i].items.length; j++){
              this.computedOrders[i].push(await this.$store.dispatch('readProduct', this.orders[i].items[j]));
            }
        }
      }
  }
